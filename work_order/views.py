...
def new_work_order(request):
    ...
    if request.method == "GET":
        ...
        if not request.POST.get("cod_cli").isnumeric() or not  request.POST.get("cod_tecnico").isnumeric() or not request.POST.get("cod_ser").isnumeric():
            messages.add_message(request, constants.ERROR, 'Verifique os dados inseridos')
            cod_ser = work_order_model.objects.get(cod_user=id).nome_user,
...
def edit_work_order(request, id):
    ...
    if request.method == "GET":
        return render(request, 'edit_work_order.html', { 
            ...
            'list_employee':list_employee, 
            'list_service':list_service,
            'cod_cli' :work_order_model.objects.get(id=id).cod_cli,
            ...
            'data_alteracao':work_order_model.objects.get(id=id).data_alteracao                                                 
        })
